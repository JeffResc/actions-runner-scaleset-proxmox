...
// TestCluster_RaftRequiresAdminAPITLS pins the security gate
// added by #242: when cluster.mode=raft AND the admin API is
// enabled (http_addr non-empty), admin_api.tls must be set so
// the bearer secret is encrypted on every standby→leader
// forward hop. Without this check an operator could bring up a
// raft cluster with the shared secret traversing the cluster
// subnet in cleartext.
func TestCluster_RaftRequiresAdminAPITLS(t *testing.T) {
	setEnv(t, map[string]string{
		"TEST_GH_TOKEN":  "x",
		"TEST_PVE_TOKEN": "y",
		"HOSTNAME":       "replica-a",
...
`
	_, err := config.Parse([]byte(yaml))
	require.Error(t, err)
	require.Contains(t, err.Error(), "admin_api.tls is required when cluster.mode=raft")
}

// TestCluster_RaftWithoutAdminAPI_DoesNotRequireTLS: raft mode
// without the admin API enabled (no http_addr) does NOT require
// TLS — there is no admin secret to leak in that case.
func TestCluster_RaftWithoutAdminAPI_DoesNotRequireTLS(t *testing.T) {
	setEnv(t, map[string]string{
		"TEST_GH_TOKEN":  "x",
		"TEST_PVE_TOKEN": "y",
		"HOSTNAME":       "replica-a",
...
`
	_, err := config.Parse([]byte(yaml))
	require.NoError(t, err, "raft mode without admin API must NOT require TLS")
}

// TestCluster_RaftWithAdminAPITLS_Passes: the happy path —
// raft + admin API + TLS — must parse cleanly.
func TestCluster_RaftWithAdminAPITLS_Passes(t *testing.T) {
...
// TestCluster_StandaloneWithoutTLS_OK: standalone mode keeps
// TLS optional for lab use — the bearer secret never traverses
// the network in a single-node deployment.
func TestCluster_StandaloneWithoutTLS_OK(t *testing.T) {
	setEnv(t, map[string]string{"TEST_GH_TOKEN": "x", "TEST_PVE_TOKEN": "y"})
	yaml := validPATYAML + `admin_api:
  http_addr: ":8080"
  shared_secret: secret
`
	_, err := config.Parse([]byte(yaml))
	require.NoError(t, err, "standalone mode must NOT force TLS")
}
